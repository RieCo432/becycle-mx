...
class BugReport(Base):
    ...
    reportedByUser: Mapped["User"] = relationship("User", foreign_keys=[reportedByUserId], back_populates="bugReports")
    reportedAt: Mapped[datetime] = mapped_column("reportedat", DateTime, nullable=False, default=lambda _: datetime.now(timezone.utc), server_default=text("(current_timestamp at time zone 'utc')"), quote=False)
    ...
    def reportedByUserName(self):
        return self.reportedByUser.username
