...
export default {
  ...
  async created() {
    ...
    const timeRange = (await requests.getCalendarTimeRange()).data;
    this.openTime = moment(new Date().setHours(...timeRange['open_time'].split(':')))
      .subtract(this.slotDurationMinutes, 'minutes').format('HH:mm:ss');
    this.closeTime = moment(new Date().setHours(...timeRange['close_time'].split(':')))
