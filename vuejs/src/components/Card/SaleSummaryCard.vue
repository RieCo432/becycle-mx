...
<template>
  <Card
      :title="
      saleHeader.transactionHeaderId ?
      `Closed By
        ${saleHeader.transactionHeader?.postedByUser.username}
        @ ${new Date(Date.parse(saleHeader.transactionHeader.postedOn))
          .toLocaleDateString(undefined, { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric'})}` :
      `Started By ${saleHeader.createdByUser.username}
        @ ${new Date(Date.parse(saleHeader.createdOn))
          .toLocaleDateString(undefined, { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric'})}`"
          ...
          <div class="w-full flex-shrink">
            <div class="grid grid-cols-11 gap-2 divide-x divide-y dark:text-slate-300 text-slate-700 align-middle">
              ...
              <div class="col-span-5 grid grid-cols-5 gap-2 divide-x divide-y dark:text-slate-300 text-slate-700 align-middle">
                ...
                <div class="col-span-full h-8"
                     v-for="i in (maximumNumberOfSaleLines - numberOfItemSaleLines + 1)"
                     :key="i"/>
                <div class="col-span-4 text-center h-8">
                  <h5>Sub Total</h5>
                </div>
                <div class="col-span-1 text-right h-8">
                  <h5>{{ (saleHeader.catalogueItemSaleLines
                    .reduce((total, line) => total + line.salePrice, 0) / 100)
                    .toFixed(2) }}</h5>
                </div>
              </div>
              ...
              <div class="col-span-5 grid grid-cols-5 gap-2 divide-x divide-y dark:text-slate-300 text-slate-700 align-middle">
                ...
                <template v-for="line in saleHeader.bikeSaleLines"
                          :key="line.id">
                  <div class="col-span-1 h-8">
                    <span>{{ line.bike.make }}</span>
                  </div>
                  <div class="col-span-1 h-8">
                    <span>{{ line.bike.model }}</span>
                  </div>
                  <div class="col-span-1 h-8">
                  ...
                  <div class="col-span-1 text-left h-8">
                    <span>{{ line.bike.serialNumber }}</span>
                  </div>
                  <div class="col-span-1 text-right h-8">
                    <span class="inline align-bottom mb-0">{{ (line.salePrice / 100).toFixed(2) }}</span>
                  </div>
                ...
                <div class="col-span-full h-8"
                     v-for="i in (maximumNumberOfSaleLines - numberOfBikeSaleLines + 1)"
                     :key="i"></div>
                <div class="col-span-4 text-center h-8">
                  <h5>Sub Total</h5>
                </div>
                <div class="col-span-1 text-right h-8">
                  <h5>{{ (saleHeader.bikeSaleLines
                    .reduce((total, line) => total + line.salePrice, 0) / 100)
                    .toFixed(2) }}</h5>
                </div>
              </div>
              <div class="col-span-full h-8"/>
              <div class="col-span-9 col-start-1 text-center h-8">
                <h5>Total</h5>
              </div>
              <div class="col-span-2 col-start-10 text-right h-8">
                <h5>{{ (
                         (
                           saleHeader.catalogueItemSaleLines
                             .reduce((total, line) => total + line.salePrice, 0) +
                           saleHeader.bikeSaleLines
                             .reduce((total, line) => total + line.salePrice, 0)
                         ) / 100
                       ).toFixed(2) }}</h5>
              </div>
        ...
        <div class="flex flex-col">
          <table
            ...
            <tr class=" dark:bg-slate-700">
              <th class="border dark:border-slate-500">
                Account
              </th>
              <th class="border dark:border-slate-500">
                Credit
            ...
            <tr v-for="line in saleHeader.transactionHeader.transactionLines"
                :key="line.id">
